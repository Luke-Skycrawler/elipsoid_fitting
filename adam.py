...
@wp.kernel
def adam_step_kernel_vec2(
    g: wp.array(dtype=wp.vec2),
    m: wp.array(dtype=wp.vec2),
    ...
    mhat = m[i] / (1.0 - wp.pow(beta1, (t + 1.0)))
    vhat = v[i] / (1.0 - wp.pow(beta2, (t + 1.0)))
    sqrt_vhat = wp.vec2(wp.sqrt(vhat[0]), wp.sqrt(vhat[1]))
    eps_vec3 = wp.vec2(eps)
    params[i] = params[i] - lr * wp.cw_div(mhat, (sqrt_vhat + eps_vec3))

@wp.kernel
def adam_step_kernel_mat22(
    g: wp.array(dtype=wp.mat22),
    m: wp.array(dtype=wp.mat22),
    v: wp.array(dtype=wp.mat22),
    lr: float,
    beta1: float,
    beta2: float,
    t: float,
    eps: float,
    params: wp.array(dtype=wp.mat22),
):
    i = wp.tid()
    m[i] = beta1 * m[i] + (1.0 - beta1) * g[i]
    v[i] = beta2 * v[i] + (1.0 - beta2) * wp.cw_mul(g[i], g[i])
    ...
    vhat = v[i] / (1.0 - wp.pow(beta2, (t + 1.0)))
    sqrt_vhat = wp.mat22(
        wp.sqrt(vhat[0, 0]), wp.sqrt(vhat[0, 1]),
        wp.sqrt(vhat[1, 0]), wp.sqrt(vhat[1, 1])
    )    
    eps_vec3 = wp.mat22(eps)
    params[i] = params[i] - lr * wp.cw_div(mhat, (sqrt_vhat + eps_vec3))
...
class Adam:
    ...
    def step_detail(g, m, v, lr, beta1, beta2, t, eps, params):
        ...
        elif params.dtype == wp.types.vec2:
            wp.launch(
                kernel=adam_step_kernel_vec2,
                dim=len(params),
                inputs=kernel_inputs,
                device=params.device,
            )
        elif params.dtype == wp.types.mat22:
            wp.launch(
                kernel=adam_step_kernel_mat22,
                dim=len(params),
                inputs=kernel_inputs,
                device=params.device,
            )
